...
class GraphStore:
    ...
    def add_entity(self, entity: Entity) -> None:
        self.conn.execute(
            f"CREATE (n:Entity {{id:{self._q(entity.id)}, kind:{self._q(entity.kind)}, "
    ...
    def add_fact(self, fact: Fact) -> None:
        negated = "true" if fact.negated else "false"
        self.conn.execute(
            f"CREATE (n:Fact {{id:{self._q(fact.id)}, kind:{self._q(fact.kind)}, "
            ...
            f"valid_to:{self._q(fact.valid_to)}, negated:{negated}, "
    ...
    def add_rule(self, rule: Rule) -> None:
        self.conn.execute(
            f"CREATE (n:Rule {{id:{self._q(rule.id)}, kind:{self._q(rule.kind)}, "
            f"text:{self._q(rule.text)}, rule_type:{self._q(rule.rule_type)}, "
            f"priority:{rule.priority}, created_at:{self._q(rule.created_at)}, "
            f"updated_at:{self._q(rule.updated_at)}, confidence:{rule.confidence}, "
            f"source:{self._q(rule.source)}, source_ref:{self._q(rule.source_ref)}, "
            f"status:{self._q(rule.status)}}})"
        )
    ...
    def find_entity_by_canonical_key(self, canonical_key: str) -> dict | None:
        ...
        if not rows:
            return None
        return {"id": rows[0][0], "name": rows[0][1], "canonical_key": rows[0][2]}
